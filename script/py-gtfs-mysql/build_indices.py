...
def stop_times(connection):
    ...
    while row is not None:
        ...
        stop_id = row[3]
        sql = "UPDATE stop_times SET arrival_time_seconds=%d, departure_time_seconds=%d WHERE trip_id='%s' AND stop_id='%s' AND arrival_time='%s' AND departure_time='%s'" % (convert_time_string_to_seconds(arrival_time), convert_time_string_to_seconds(departure_time), trip_id, stop_id, arrival_time, departure_time)
        insert_cursor.execute(sql)
        row = cursor.fetchone()
